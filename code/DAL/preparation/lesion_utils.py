...
def extract_lesion_candidates(
    ...
    # Dark lesion-like regions
    blur = cv2.GaussianBlur(gray, (5, 5), 0)
    _, dark_mask = cv2.threshold(
        blur, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU
    )

    # Combine lesion evidence
    ...
    # Fallback: full image if no lesion candidate found
    if len(candidates) == 0:
    ...
    for _, x, y, w, h in candidates:
        pad_x = int(LESION_CONTEXT_PAD_RATIO * w)
        pad_y = int(LESION_CONTEXT_PAD_RATIO * h)
